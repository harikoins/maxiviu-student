  ...
  return (
    <>
      <Card
        ...
        title={
          <div className="d-flex justify-content-between align-items-center">
            <Typography.Title level={5}>Competency Completeness</Typography.Title>
            <a href="#" style={{ color: "#4DC4FF" }}>View Insights</a>
          </div>
        }
      >
      ...
      >
        {documents.map((doc, index) => (
          <div key={index} className="d-flex justify-content-between align-items-center border-bottom py-2">
            ...
            <Button type="primary" shape="round">{doc.action}</Button>
          </div>
        ))}
      </Card>
      ...
      <Card
        title={
          <div className="d-flex justify-content-between align-items-center">
            <Typography.Title level={5} style={{ margin: 0 }}>Projects & Innovations</Typography.Title>
            <Button shape="circle" icon={<PlusOutlined />} />
          </div>
        }
        bordered={false}
      ...
      >
        {projects.map((project, index) => (
          <div key={index} className="d-flex justify-content-between align-items-center border-bottom py-2">
            ...
            <Button type="primary" shape="round">{project.action}</Button>
          </div>
        ))}
        <Typography.Link style={{ color: "#4DC4FF" }}>Show more</Typography.Link>
      </Card>

      <Card
        title={<Typography.Title level={5} style={{ margin: 0 }}>Events/Campus</Typography.Title>}
        bordered={false}
        className="mt-3"
      >
        {events.map((event, index) => (
          <div key={index} className="d-flex justify-content-between align-items-center border-bottom py-2">
            <div>
              ...
              <Typography.Text type="warning">{event.date} {event.time}</Typography.Text>
            </div>
